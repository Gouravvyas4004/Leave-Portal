...
function put(channel3, action) {
  {
    if (arguments.length > 1) {
      check(channel3, notUndef, "put(channel, action): argument channel is undefined");
      check(channel3, channel, "put(channel, action): argument " + channel3 + " is not a valid channel");
      check(action, notUndef, "put(channel, action): argument action is undefined");
    } else {
      check(channel3, notUndef, "put(action): argument action is undefined");
    }
  }
  if (undef(action)) {
    action = channel3;
...
function actionChannel(pattern2, buffer3) {
  {
    check(pattern2, _pattern, "actionChannel(pattern,...): argument pattern is not valid");
    if (arguments.length > 1) {
      check(buffer3, notUndef, "actionChannel(pattern, buffer): argument buffer is undefined");
      check(buffer3, buffer, "actionChannel(pattern, buffer): argument " + buffer3 + " is not a valid buffer");
    }
  }
...
function flush(channel3) {
  {
    check(channel3, channel, "flush(channel): argument " + channel3 + " is not valid channel");
  }
  return makeEffect(FLUSH, channel3);
}
...
//# sourceMappingURL=chunk-GOB2QELV.js.map
